...
listwise vs pointwise = 验证「排序目标」这步; {full} vs {O_k only} = 验证「context」这步; 编码方式(标量/关系/图) = 验证「怎么编码结构」; g_net vs PageRank/HITS = 验证「用净度数而非中心性」; 真图 vs 打乱图 = 验证「结构真实性」; 残差化 vs 原始 = 验证「去混淆」这步. 因此下表每一行的身份都是"某步骤的验证", 步骤本身属于方法(§2), 这里只报验证结果.
