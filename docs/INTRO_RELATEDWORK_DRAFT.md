...
**P0 — Motivating scenario.** Consider an agentic market-intelligence pipeline ingesting the cashtag firehose of X: thousands of *freshly-originated* financial narratives arrive per day, while a fixed daily budget of expensive downstream reasoning (LLM deep-reading, hypothesis generation, analyst attention) can be spent on only a few. The obvious "let an LLM read the stream" solution collapses for two reasons. (i) *Cost* scales (super)linearly with the tokens read — at firehose volume the LLM cannot keep up. (ii) *Dilution*: handing a large candidate pool to an LLM makes its selection **worse**, not better — in our experiments the LLM's reach-capture is *lowest* when it reads the full pool (0.455) and rises to 0.636 once a cheap pre-filter shortlists for it. What the pipeline needs is a **cheap, zero-text gate at the moment of origination** that ranks narratives by their eventual reach *before any popularity is observable*, so the expensive model only ever sees a high-value shortlist — a gate that doubles as an early-breakout alarm. This paper builds that gate.
