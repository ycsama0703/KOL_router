...
THR = 0.50; MEK = 8; TARGET = "log_future_adopt"; N_GRID = [50, 100, 200, 400]
...
def main():
    ...
    sets = {"A_context": base, "B_ctx+Ok": base + ["origin_ol"], "G_ctx+graph": base + GRAPH,
            "BG_ctx+Ok+graph": base + ["origin_ol"] + GRAPH}
    ev = {k: [] for k in sets}
    ...
    pairs = [("G_ctx+graph", "A_context"), ("BG_ctx+Ok+graph", "A_context"), ("BG_ctx+Ok+graph", "B_ctx+Ok"), ("G_ctx+graph", "B_ctx+Ok")]
    comps = p65.comparisons(ev, pairs)
    log("=== pooled bootstrap CI (adopt target, B=4000, 95% CI) ===")
    for mdl, b in pairs:
    ...
    pathlib.Path(__file__).with_name("phase101_graph_adopt_result.json").write_text(json.dumps({"means": means, "comparisons": comps, "elapsed_sec": time.time() - t0}, indent=2))
