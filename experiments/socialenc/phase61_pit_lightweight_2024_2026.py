"""Phase61: PIT lightweight main + ablation test on the recent 2024-2026 window.
...
Protocol:
  ...
  - Train:      2022-06-01 <= day < 2023-06-01
  - Validation: 2023-06-01 <= day < 2024-06-01
  - Test:       2024-06-01 <= day < 2026-06-01

For each block, originator role O_k and historical origin statistics are
estimated only from data before that block starts:
  - train rows use history before 2022-06-01
  - validation rows use history before 2023-06-01
  - 2024-2025 test rows use history before 2024-06-01
  - 2025-2026 test rows use history before 2025-06-01
...
OUT_JSON = pathlib.Path(__file__).with_name("phase61_pit_lightweight_2024_2026_result.json")
OUT_MD = pathlib.Path(__file__).with_name("phase61_pit_lightweight_2024_2026_table.md")
...
def block_for_day(day: str) -> str | None:
    if TRAIN_START <= day < TRAIN_END:
        return "train"
    if TRAIN_END <= day < VAL_END:
        return "val"
    if VAL_END <= day < TEST_MID:
        return "test_2024_2025"
    if TEST_MID <= day < TEST_END:
...
def cutoff_for_block(block: str) -> str:
    return {
        "train": TRAIN_START,
        "val": TRAIN_END,
        "test_2024_2025": VAL_END,
        "test_2025_2026": TEST_MID,
    }[block]
...
def compute_block_state(rows_by, emb_by, all_rows):
    states = {}
    for block in ["train", "val", "test_2024_2025", "test_2025_2026"]:
...
def fit_select_score(rows, features, target):
    ...
    for alpha in RIDGE_ALPHAS:
        pred = ridge_predict_from_mask(rows, features, target, train, alpha)
        if pred is None:
            continue
        evs = event_rows_for_split(rows, pred, target, "val")
    ...
    if alpha_scores:
        ...
        best_alpha_f = 3.0
    final = ridge_predict_from_mask(rows, features, target, train_val, best_alpha_f)
...
def evaluate_methods(rows, feature_sets):
    ...
    for name, feats in feature_sets.items():
        ...
        if pred is not None:
            scores[name] = pred
            selection[name] = sel
            log(f"  {name:<22} alpha={sel['selected_alpha']:g} val_best={max(sel['val_ndcg3_by_alpha'].values()) if sel['val_ndcg3_by_alpha'] else float('nan'):.3f}")
...
def main():
    ...
    result = {
        "task": "pit_lightweight_main_ablation_2024_2026",
        "protocol": {
            "train": [TRAIN_START, TRAIN_END],
            "validation": [TRAIN_END, VAL_END],
            "test": [VAL_END, TEST_END],
            "test_blocks": {
                "test_2024_2025": [VAL_END, TEST_MID],
                "test_2025_2026": [TEST_MID, TEST_END],
            },
            "history_cutoffs": {block: states[block]["cutoff"] for block in states},
            "semantic_threshold": THR,
            "origin_window": ORIGIN_WINDOW,
            "target": TARGET,
            "alpha_selection": "select alpha by validation NDCG@3, then refit on train+validation for final test",
        },
