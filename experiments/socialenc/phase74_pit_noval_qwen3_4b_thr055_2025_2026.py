"""Phase74: PIT no-validation Qwen3-4B benchmark at thr=0.55 for 2025-2026.

This uses the same point-in-time protocol as phase61:
  - train:      2022-06-01 to 2025-06-01
  - validation: none
...
import phase72_pit_noval_lightweight_thr055_2025_2026 as p61

OUT_JSON = pathlib.Path(__file__).with_name("phase74_pit_noval_qwen3_4b_thr055_2025_2026_result.json")
OUT_MD = pathlib.Path(__file__).with_name("phase74_pit_noval_qwen3_4b_thr055_2025_2026_table.md")
...
def fit_select_matrix_score(rows: list[dict], X: np.ndarray):
    ...
    train = splits == "train"
    final = p28.ridge_predict(X, y, train & np.isfinite(y), train & np.isfinite(y), p61.FIXED_ALPHA)
    return final, {"selected_alpha": p61.FIXED_ALPHA, "selection": "fixed_no_validation"}
...
def write_md(result):
    ...
    lines = [
        "# Phase74 PIT No-Val Qwen3-4B Benchmark thr=0.55 2025-2026",
        "",
...
def main():
    ...
    comparisons = {}
    phase61_path = pathlib.Path(__file__).with_name("phase72_pit_noval_lightweight_thr055_2025_2026_result.json")
    ...
    result = {
        "task": "pit_noval_qwen3_4b_origin_text_thr055_2025_2026",
        "protocol": {
            ...
            "train_blocks": {
                ...
                "train_2023_2024": [p61.TRAIN_MID, p61.TRAIN_MID2],
                "train_2024_2025": [p61.TRAIN_MID2, p61.TRAIN_END],
            },
            "test_blocks": {
