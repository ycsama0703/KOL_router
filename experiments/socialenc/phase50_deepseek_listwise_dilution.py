"""Phase50: listwise DeepSeek dilution and OL routing diagnostic.
...
OUT = pathlib.Path(__file__).with_name("phase50_deepseek_listwise_dilution_result.json")
TABLE_OUT = pathlib.Path(__file__).with_name("phase50_deepseek_listwise_dilution_table.md")
CACHE_DIR = pathlib.Path(__file__).with_name("phase50_deepseek_listwise_cache")
KEY_FILE = pathlib.Path.home() / ".config/alphagap/deepseek_api_key"

DEEPSEEK_URL = "https://api.deepseek.com/chat/completions"
MODEL = "deepseek-v4-flash"
SLUG = "deepseek_official__deepseek-v4-flash__listwise_dilution_v1"
THRESHOLD = 0.55
...
def get_api_key() -> str:
    ...
    raise RuntimeError("DEEPSEEK_API_KEY is not set and key file is missing")
...
def request_deepseek(api_key: str, prompt: str, max_tokens: int) -> tuple[str, dict]:
    body = {
        "model": MODEL,
        "messages": [
...
def call_deepseek(api_key: str, prompt: str, max_tokens: int) -> tuple[str, dict, float, str]:
    last_error = ""
    for attempt in range(3):
        started = time.perf_counter()
        try:
            content, obj = request_deepseek(api_key, prompt, max_tokens)
...
def main() -> None:
    ...
    for i, task in enumerate(tasks, 1):
        ...
        if key in cache:
            ...
            content, obj, latency_ms, error = call_deepseek(api_key, prompt, max_tokens)
            calls += 1
    ...
    result = {
        "task": "deepseek_listwise_dilution",
        "model": MODEL,
        "threshold": THRESHOLD,
