"""Phase103: SHUFFLE control for graph features. Permute KOL->graph-feature-vector mapping
(parallel to shuffled-O_k). Test BG(real graph) vs BGshuf(shuffled graph). Pooled 5 windows, 90% CI.
...
p65.BOOTSTRAP_B = 4000; p65.RNG = np.random.default_rng(103)
WINDOWS = [
...
GKEYS = ["g_out", "g_in", "g_net", "g_pr", "g_hub"]
SKEYS = ["gs_out", "gs_in", "gs_net", "gs_pr", "gs_hub"]
...
def graph_feats(all_rows, cutoff):
    ...
    return feats
def shuffled_graph(feats, block):
    keys = sorted(feats); vals = [feats[k] for k in keys]
    seed = 10300 + sum(ord(c) for c in block)
    perm = np.random.default_rng(seed).permutation(len(vals))
    return {k: vals[perm[j]] for j, k in enumerate(keys)}
def attach(panel, gf, gfs):
    zero = {kk: 0.0 for kk in GKEYS}
    for r in panel:
        b = r["split"]; g = gf[b].get(r["origin_kol"], zero); gs = gfs[b].get(r["origin_kol"], zero)
        for kk in GKEYS: r[kk] = g[kk]
        for kk in GKEYS: r["gs_" + kk[2:]] = gs[kk]
...
def main():
    ...
    base = p7.FEATURE_SETS["no_ol_strong"] + ["origin_ol"]
    sets = {"A_context": p7.FEATURE_SETS["no_ol_strong"], "BG_realgraph": base + GKEYS, "BGshuf_graph": base + SKEYS}
    ev = {k: [] for k in sets}
    for lab, tr_s, te_s, te_e, inner in WINDOWS:
        ...
        gfs = {"train": shuffled_graph(gf["train"], "train"), "test": shuffled_graph(gf["test"], "test")}
    ...
    for k in sets:
        mm = means[k]; log("  %-16s NDCG=%.4f Hit=%.4f" % (k, mm["ndcg3"], mm["hit1"]))
    comps = p65.comparisons(ev, [("BG_realgraph", "BGshuf_graph"), ("BG_realgraph", "A_context"), ("BGshuf_graph", "A_context")])
    log("=== bootstrap 90% CI ===")
    for mdl, b in [("BG_realgraph", "BGshuf_graph"), ("BG_realgraph", "A_context"), ("BGshuf_graph", "A_context")]:
        for metric in ["ndcg3", "hit1"]:
            ...
            log("  %-32s %-6s %+0.4f [%+.4f, %+.4f] %s" % ("%s - %s" % (mdl, b), metric, s["observed"], s["ci05"], s["ci95"], sig))
    pathlib.Path(__file__).with_name("phase103_graph_shuffle_control_result.json").write_text(json.dumps({"means": means, "comparisons": comps}, indent=2))
